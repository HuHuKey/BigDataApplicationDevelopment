...
//左边成交动态板块切换
for (var i = 0; i < CJstatus.length; i++) {
    $(".liushuihaoul .moveul").html((index, html) => {
        return html += `<li>
				<span>${CJstatus[i].commentNum}</span>
				<span>${CJstatus[i].name}</span>
				<span>${CJstatus[i].price}</span>
				<span>${CJstatus[i].totalSale}</span>
				<span>${CJstatus[i].supplier}</span>
    ...
    $(this).click(function () {
        ...
        for (var i = 0; i < Status.length; i++) {
            $(".liushuihaoul .moveul").html((ind, html) => {
                return html += `<li>
				<span>${Status[i].commentNum}</span>
				<span>${Status[i].name}</span>
				<span>${Status[i].price}</span>
				<span>${Status[i].totalSale}</span>
				<span>${Status[i].supplier}</span>
				<span>${Status[i].href}</span>
				</li>`
            })
        }
        siz2 = $(".liushuihaoul .moveul").find("li").length;
