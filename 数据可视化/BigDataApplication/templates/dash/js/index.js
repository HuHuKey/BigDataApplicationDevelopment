...
for (var i = 0; i < CJstatus[1].length; i++) {
	$(".liushuihaoul .moveul").html((index, html) => {
		return html += `<li>
				<span>${CJstatus[1][i].rank}</span>
				<span>${CJstatus[1][i].name}</span>
				<span>${CJstatus[1][i].price}</span>
				<span>${CJstatus[1][i].origin}</span>
				<span>${CJstatus[1][i].supplier}</span>
				<span>${CJstatus[1][i].selling}</span>
				</li>`
	})
}
	...
	$(this).click(function() {
		...
		for (var i = 0; i < Status.length; i++) {
			$(".liushuihaoul .moveul").html((ind, html) => {
				return html += `<li>
				<span>${Status[i].rank}</span>
				<span>${Status[i].name}</span>
				<span>${Status[i].price}</span>
				<span>${Status[i].origin}</span>
				<span>${Status[i].supplier}</span>
				<span>${Status[i].selling}</span>
				</li>`
			})
		}
		siz2 = $(".liushuihaoul .moveul").find("li").length;
